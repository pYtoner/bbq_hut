...
int circularIndex(int idx, Side side, Direction direction) {
  int tapestry_idx = idx / sideLengths[(int)side];

  int side_idx = idx % sideLengths[(int)side];

  int transformed_idx = sideIndex(tapestry_idx, side_idx, side, direction);
...
void circlingDot() {
  ...
  while (true) {
...

    {
      int side = 3;
      int side_idx = (int)(tapestry_progress * sideLengths[side]);
      int transformed_idx = sideIndex(tapestry_idx, side_idx, side, 1);
      leds[transformed_idx] = CRGB(255, 0, 0);
    }
    FastLED.show();
    changeIdx += 1;
...
void circlingPulse() {
  ...
  while (true) {
    ...
    for (int t = 0; t <= n_dots - 1; t++) {
      int i = (changeIdx + t + 54) % maximum;
      int transformed_idx = circularIndex(i, side, FORWARDS);
      ...
      int i2 = (changeIdx + t) % maximum;
      int transformed_idx2 = circularIndex(i2, RIGHT, BACKWARDS);
      leds[transformed_idx2] = CRGB(color, 0, 0);
    }
    FastLED.show();
    changeIdx += 1;
  }
...
void circlingPulse2() {
  ...
  while (true) {
    ...
    for (int t = 0; t <= n_dots - 1; t++) {
      ...
      // int transformed_idx2 = circularIndex(i % maximum, RIGHT, FORWARDS);
      // leds[transformed_idx2] = CRGB(color, 0, 0);
    }
    FastLED.show();
    changeIdx += 1;
